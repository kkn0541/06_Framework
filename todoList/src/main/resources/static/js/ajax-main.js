...
changeComplete.addEventListener("click",() =>{
  ...
  fetch("/ajax/changeComplete",{
    method:"PUT", // 서버단에서는 @PutMapping  사용 
    headers : {"Content-Type" : "application/json"},
    body:JSON.stringify(obj)  // js objsct 형태인 obj를 json형식으로 변경 

  })

  .then(resp => resp.text())
  //resp에  contorller에서return 값 담김 
  .then(result => {
 ...
 if(result >0){ // 성공
  ...
  //getCompleteCount();
    ...
    //innertext 로 가져온것은 문자열형태  + - 해줄려면 숫자형으로 형변환해야함
...
//상세조회에서 숭정 버튼(#updateView) 클릭 시 

updateView.addEventListener("click",()=>{

  //기존 상세 조회 팝업 레이어는 숨기고 
  popupLayer.classList.add("popup-hidden"); //팝업 사라짐 
...
//(상세조회)팝업 레이어에 작성된 제목 , 내용 얻어와 세팅 
updateTitle.value= popupTodoTitle.innerText;

//HTML 화면에서 줄바꿈이 br로 인식되고 있는데
// TEXTAREA에서는 \n 으로 바꿔줘야 줄 바꿈으로 인식된다. 
