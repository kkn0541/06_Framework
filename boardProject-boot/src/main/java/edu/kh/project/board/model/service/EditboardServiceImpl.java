...
public class EditboardServiceImpl implements EditBoardService {
	...
	@Value("${my.board.foler-path}") // C:/uploadFiles/board/
	private String folderPath;

	@Override
	public int boardInsert(Board inputBoard, List<MultipartFile> images) throws Exception {

		// 1. 게시글 부분 (inputBoard)를 먼저
		// Board 테이블 insert 하기
		// ->Insert 결과로 작성된 게시글 번호(시퀀스 반환) 받기

		int result = mapper.boardInsert(inputBoard);

		// result == INSERT 결과 (삽입 성공한 핸의 개수 0 OR 1
		...
		// baoardNo가 inputBoard에 저장된 상태 ! 얕은 복사 개념 이해 필수
		...
		for (int i = 0; i < images.size(); i++) {
			...
			if (!images.get(i).isEmpty()) {
				...
				// 모든 값을 저장할 DTO 생성 (BoardImg -Bulder 패턴 사용 )
	...
	public int boardUpdate(Board inputBoard, List<MultipartFile> images, String deleteOrderList) throws Exception{
		...
		for (int i = 0; i < images.size(); i++) {
			...
			if (!images.get(i).isEmpty()) {
				...
				// 1) 기존에 있었는데 새 이미지로 변경하는 경우 수정 해야함 
				//     기존 o -> 새 이미지로 변경 -> 수정 
