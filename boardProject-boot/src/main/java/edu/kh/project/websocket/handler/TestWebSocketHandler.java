...
/**000 websocket 핸들러 클래스 
 * 웹소켓 동작시 수행할 구문을 작성한느 클래스 
 * 
 */
@Slf4j
@Component // bean 등록 
public class TestWebSocketHandler extends TextWebSocketHandler {
	...
	//sessionhandlerInterCEPTOR 가 가로챈 
	// 연결한 클라이언트의 HTTPSESSION 을 가지고 있음 
	// (attributs 에 추가한 값 
	
	//동기화 된 set 생성 
	private Set<WebSocketSession> sessions = Collections.synchronizedSet(new HashSet<>());
	//여러가지 스레드가 동작하는 환경에서 하나의 컬렉션에 여러 스레그 접근하여 의도치 않은 문제가 발생되지 않게 
	// 하기 위해 동기화를 진행하여 스레드가 순서대로 한켈력션에 접근할수있도록 변경 
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		//연결된 클라이언트의 websoecketsesiion 정보를 set에 추가 
		// -> 웹소캣에 연결된 클라이언트 정보를 모아둠 
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		//웹소캣 연결이 끊긴 클라리언트 정보를 set 에서 제거 
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		//TextMessage :웹 소켓으로 연결된 클라이언트가 전달한 
		//  		텍스트(내용) 가 담겨있는 객체 
