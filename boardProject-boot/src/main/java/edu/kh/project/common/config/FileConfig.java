...
public class FileConfig implements WebMvcConfigurer {
	...
	// 스프링 구성을 커스터마지이하고 확장하기 위한 메서드를 제공함 
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		...
		// Spring Mvc에서 정적 리소스 (이미지 , css js 파일 등)의 요청을 처리하기 위해
		// 사용하는 클래스 
		
		//URL 요청 패튼을 서버의 실제 파일 경로와 연결하여 ,
		//클라이언트가 특정 경로로 정적 파일에 접근할수있도록 설정 
	...
	// 파일업로드를 처리하는데 사용되는 MultipartConfigElement 를 구성하고 반환 
	...
	public MultipartResolver multipartResolver() {
		...
		// MultipartResolver는 클라이언트로부터 받은 multipart요청을 처리하고 , 
