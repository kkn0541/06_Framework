...
public class ParameterController {
	 ...
	 *  
	 *  required :입력된 name속성값 파라미터 필수 여부 지정 (기본값 true ) 
	...
	//@RequestParam 이용시 참고사항
	// EX) form 태그 안에서 input 태그를 이용해서 테스트 중 
	// input태그 안에 값 작성 하지 않더라도 
	// 파라미터로는 title="" & writer ="" &....
	//	-> 파라미터가 없는게 아님 
	// 	-> required= true 에 위배되지 않는 것 
	...
	public String paramTest3(
		...
		// -> 같은 name속성 파라미터가 StRING[],lIST 로 저장 X 
		// String[] , List 형태라면 ㅓ처음에 체크한 값이 넘어온다 
	...
	// 같이 사용되는 dto 필드명과 같다면
	// 자동으로 setter 를 호출해서 필드에 값을 세팅 
	
	//** 주의사항ㅎ *****
	//Dto에 기본 생성자가 필수로 존재햐아 한다. 
	//Dto에 setter 가  필수로 존재햐아 한다 
	
	//@ModelAttribute 를 이용해 값이 필드에 세팅된 객체를
	// "커맨드 객체"라고 부른다. 
	//@ModelAttribute 어노테이션 생략 가능
	@PostMapping("test4")   // param /test4로  post 요청 매핑 
